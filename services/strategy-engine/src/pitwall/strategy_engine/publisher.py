...
class StrategyPublisher:
    """Publishes deterministic strategy events back to the Canonical stream."""
    def __init__(self):
    ...
    def publish(self, event: PitWallEvent, is_live=False):
        try:
            ...
            logger.debug(f"Published Strategy Event {event.event_type} for {event.session_id}")
            return msg_id
        except Exception as e:
            logger.error(f"Failed to publish Strategy Event: {e}")
